...
from utils import format_scene_plan_for_prompt # Assuming this utility exists or will be created
...
class DraftingAgent:
    ...
    async def draft_chapter(
        ...
        scene_plan_prompt_text = "No detailed scene plan available. Focus on the plot point."
        ...
        prompt_lines.extend([
            ...
            f"- Adhere closely to the 'Detailed Scene Plan' if provided. Each scene should flow logically to the next.",
            f"- If no detailed scene plan is available, use the 'Primary Plot Point Focus' to structure the chapter with a clear beginning, middle, and end, incorporating multiple distinct scenes or events as appropriate to achieve the plot point.",
        ...
        available_for_generation = config.MAX_CONTEXT_TOKENS - prompt_tokens - 200 # 200 as buffer
        ...
        if available_for_generation < max_gen_tokens :
             if available_for_generation > 500: # Min reasonable generation
                 ...
                 logger.info(f"Drafting Ch {chapter_number}: Adjusted max_tokens for generation to {max_gen_tokens} due to prompt size.")
             else:
                 logger.error(f"Drafting Ch {chapter_number}: Insufficient token space for generation after prompt. Prompt tokens: {prompt_tokens}, Model max context: {config.MAX_CONTEXT_TOKENS}. Cannot draft.")
                 return None, "Prompt too large for generation window.", None


        logger.info(f"Calling LLM ({self.model_name}) to draft Chapter {chapter_number}. Est. prompt tokens: {prompt_tokens}. Max generation tokens: {max_gen_tokens}.")
        ...
        if not draft_text or not draft_text.strip():
            logger.error(f"DraftingAgent: LLM returned empty or whitespace-only draft for Chapter {chapter_number}.")
            return None, draft_text, usage_data # Return raw LLM output even if it's bad

        logger.info(f"DraftingAgent: Successfully generated draft for Chapter {chapter_number}. Length: {len(draft_text)} characters.")
        return draft_text, draft_text, usage_data # For now, raw_llm_output is same as cleaned draft_text
