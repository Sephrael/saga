...
class NovelWriterAgent:
    ...
    def _save_json_state(self):
        # Same as your last refactored version (with is_default cleanup)
                ...
                if data_dict and isinstance(data_dict, dict):
                    ...
                    if "is_default" in data_to_save:
                        ...
                        if not is_default_flag_value or (is_default_flag_value and not is_content_truly_default):
                            del data_to_save["is_default"]; logger.debug(f"'is_default' flag removed from {data_dict_attr_name}.")
    ...
    def generate_plot_outline(self, default_protagonist_name: str, unhinged_mode: bool, **kwargs) -> Dict[str, Any]:
        ...
        Uses kwargs for genre, theme, setting_description OR archetypes based on unhinged_mode.
        """
        logger.info(f"Generating plot outline. Unhinged mode: {unhinged_mode}")

        # Prepare prompt based on mode
        if unhinged_mode:
            genre = kwargs.get("genre", "fantasy") # Fallbacks if not provided
...
"""
            # Store the randomized base elements for saving later
            base_elements_for_outline = {
                "genre": genre, "theme": theme, 
                "setting_archetype_used": setting_archetype, # Note these are archetypes
...
"""
            # Store the configured base elements
...
{{
  ...
  "setting": "string", // This will be generated by LLM in unhinged, taken from config in standard
  "conflict": "string",
  // genre, theme will be added programmatically
}}
"""
        logger.info("Calling LLM for plot outline generation...")
        raw_outline_str = llm_interface.call_llm(prompt, temperature=0.7) # Slightly higher temp for creativity
        ...
        required_keys_unhinged = required_keys_standard + ["setting"] # 'setting' is generated by LLM in unhinged
        ...
        if parsed_outline and isinstance(parsed_outline, dict):
            ...
            if (all(key in parsed_outline and parsed_outline[key] for key in required_keys) and
                ...
                # Add the base elements (genre, theme, etc.) that were used for generation
        ...
        if not is_valid:
            ...
            # Add base elements even for default fallback
            self.plot_outline.update(base_elements_for_outline)
        
        # Ensure protagonist_name has a fallback
    ...
    def generate_world_building(self):
        # Same as your last refactored version, ensuring prompt asks for dicts
        ...
        if not self.plot_outline or not self.plot_outline.get("setting"): # 'setting' is crucial
    ...
    # --- Chapter Planning, Writing, Evaluation, Revision, Finalization (mostly same) ---
    # ... (These methods are largely unchanged from your last provided version,
    #      as they were already well-refactored. The key changes were in generation & knowledge updates)
    def _plan_chapter(self, chapter_number: int) -> Optional[str]:
        # Same as your last version
        if not config.ENABLE_AGENTIC_PLANNING: return "Agentic planning disabled by configuration."
        logger.info(f"Planning Chapter {chapter_number}...")
        plot_point_focus, plot_point_index = self._get_plot_point_info(chapter_number)
        if plot_point_focus is None: logger.error(f"Cannot plan ch {chapter_number}: no plot point."); return None
        context_summary = ""
        if chapter_number > 1:
            prev_chap_data = self.db_manager.get_chapter_data_from_db(chapter_number - 1)
            if prev_chap_data:
                prev_summary = prev_chap_data.get('summary')
                if prev_summary: context_summary += f"Summary of Previous Chapter ({chapter_number - 1}):\n{prev_summary[:1000]}...\n"
                else:
                     prev_text = prev_chap_data.get('text', '')
                     if prev_text: context_summary += f"End Snippet of Previous Chapter ({chapter_number - 1}):\n...{prev_text[-1000:]}\n"
        kg_facts = []
        ...
        **Recent Context (End of Previous Chapter/Summary):**
        ...
        {kg_context_section}
        **Current Character States (Key Characters Only, from JSON profiles):**
        {self._get_relevant_character_state_snippet()}
        **Current World State (Relevant Locations/Elements, from JSON world-building):**
        {self._get_relevant_world_state_snippet()}
        **Task:** Outline 3-5 essential scenes/beats for Chapter {chapter_number} that:
        1. Directly address the **Mandatory Focus**. 2. Logically follow **Recent Context** & **KG Facts**. 3. Involve relevant characters/world elements. 4. Contribute to **Protagonist Arc Goal**.
        **Output Format:** ONLY a numbered list of scenes/beats (1 sentence per beat). Start with "1.".
        ...
        if plan_cleaned and re.match(r"^\s*1\.", plan_cleaned):
            logger.info(f"Successfully generated plan for chapter {chapter_number}:\n{plan_cleaned}"); return plan_cleaned
        else: logger.error(f"Failed to generate valid plan for ch {chapter_number}. Response: '{plan_raw[:200]}...'"); return None
    ...
    def write_chapter(self, chapter_number: int) -> Optional[str]:
        ...
        if not self.plot_outline or not self.plot_outline.get("plot_points") or not self.plot_outline.get("protagonist_name"):
            ...
            return None
        if chapter_number <= 0: logger.error(f"Cannot write chapter {chapter_number}: Chapter number must be positive."); return None
        chapter_plan = self._plan_chapter(chapter_number)
        if config.ENABLE_AGENTIC_PLANNING and chapter_plan is None: logger.error(f"Chapter {chapter_number} generation halted due to planning failure."); return None
        context_for_draft = self._get_context(chapter_number)
        plot_point_focus, _ = self._get_plot_point_info(chapter_number)
        initial_draft_text, initial_raw_text = self._generate_draft(chapter_number, plot_point_focus, context_for_draft, chapter_plan)
        if not initial_draft_text:
            logger.error(f"Failed to generate initial draft for chapter {chapter_number}.")
            self._save_debug_output(chapter_number, "initial_raw_fail_after_clean", initial_raw_text or ""); return None
        ...
        if evaluation["needs_revision"]:
            ...
            revised_text_tuple = self._revise_chapter(current_text, chapter_number, revision_reason_str, context_for_draft, chapter_plan)
            if revised_text_tuple:
                ...
                if revised_evaluation["needs_revision"]: logger.error(f"Revised draft for ch {chapter_number} STILL failed evaluation. Reasons:\n- " + "\n- ".join(revised_evaluation["reasons"]))
                else: logger.info(f"Revised draft for chapter {chapter_number} passed evaluation.")
                current_text, final_raw_output_log = revised_text, final_raw_output_log + f"--- REVISION ATTEMPT (Reason: {revision_reason_str}) ---\n{raw_revision_output}\n\n"
            else:
        ...
        else: logger.info(f"Initial draft for chapter {chapter_number} passed evaluation.")
        if not self._finalize_chapter_core(chapter_number, current_text, final_raw_output_log):
             logger.error(f"=== Finished Chapter {chapter_number} Generation With Errors During Core Finalization ==="); return None
    ...
    def _get_plot_point_info(self, chapter_number: int) -> Tuple[Optional[str], int]:
        ...
        if 0 <= plot_point_index < len(plot_points): return plot_points[plot_point_index], plot_point_index
        else: logger.warning(f"Could not find plot point for chapter {chapter_number}."); return None, -1
    ...
    def _generate_draft(self, chapter_number: int, plot_point_focus: Optional[str], context: str, plan: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
        # Same as your last version
        if not plot_point_focus: plot_point_focus = "Continue the narrative logically."
        plan_section = ""
        ...
        **Focus for THIS Chapter:** {plot_point_focus}
        {plan_section}
        **Current World Building Notes (JSON):** ```json\n{json.dumps(self.world_building, indent=2, ensure_ascii=False, default=str)}\n```
        **Current Character Profiles (JSON):** ```json\n{json.dumps(self.character_profiles, indent=2, ensure_ascii=False, default=str)}\n```
        **Context from Previous Relevant Chapters:**\n--- BEGIN CONTEXT ---\n{context if context else "No previous context."}\n--- END CONTEXT ---
        **Instructions:** Write a compelling chapter (1000-2000 words). Follow plan/focus. Maintain consistency. Smooth flow. Vivid prose for genre '{self.plot_outline.get('genre', 'story')}'. **Output ONLY chapter text.**
        ...
        if not cleaned_text or len(cleaned_text) < config.MIN_ACCEPTABLE_DRAFT_LENGTH:
             logger.error(f"Ch {chapter_number} draft too short ({len(cleaned_text or '')} chars)."); return None, raw_text
        logger.info(f"Generated initial draft for ch {chapter_number} (Len: {len(cleaned_text)})."); return cleaned_text, raw_text
    ...
    def _evaluate_draft(self, draft_text: str, chapter_number: int, previous_chapters_context: str) -> EvaluationResult:
        ...
        if chapter_number > 1:
            ...
            if current_embedding is not None and prev_embedding is not None:
                ...
                if coherence_score < config.REVISION_COHERENCE_THRESHOLD: needs_revision = True; reasons.append(f"Low coherence (Score: {coherence_score:.4f}).")
        ...
        if len(draft_text) < config.MIN_ACCEPTABLE_DRAFT_LENGTH: needs_revision = True; reasons.append(f"Draft too short ({len(draft_text)} chars).")
    ...
    def _revise_chapter(self, original_text: str, chapter_number: int, reason: str, context_from_previous: str, plan: Optional[str]) -> Optional[Tuple[str, str]]:
        ...
        context_snippet, original_snippet = context_from_previous[:context_limit], original_text[:original_text_limit]
        plan_focus_section = ""
        ...
        **Instructions:** 1. **PRIORITY:** Fix issues from critique. 2. **Rewrite ENTIRE chapter.** 3. Ensure logical flow with Context. 4. Preserve tone/style but prioritize fixes. 5. Aim for length >= {config.MIN_ACCEPTABLE_DRAFT_LENGTH} chars. 6. **Output ONLY rewritten chapter text.**
        ...
        original_embedding, revised_embedding = llm_interface.get_embedding(original_text), llm_interface.get_embedding(revised_cleaned)
        if original_embedding is not None and revised_embedding is not None:
            ...
            logger.info(f"Revision for ch {chapter_number} accepted (Similarity: {similarity_score:.4f})."); return revised_cleaned, revised_raw
        else: logger.warning(f"Could not get embeddings for revision similarity check of ch {chapter_number}. Accepting."); return revised_cleaned, revised_raw
    ...
    def _finalize_chapter_core(self, chapter_number: int, final_text: str, raw_log: str) -> bool:
        ...
        if not final_text: logger.error(f"Cannot finalize chapter {chapter_number}: Final text is missing."); return False
        summary = self._summarize_chapter(final_text, chapter_number)
        final_embedding = llm_interface.get_embedding(final_text)
        if final_embedding is None: logger.error(f"CRITICAL: Failed to generate embedding for final version of chapter {chapter_number}.")
        try: self.db_manager.save_chapter_data(chapter_number, final_text, raw_log, summary, final_embedding)
        except Exception as e: logger.error(f"Database save failed for chapter {chapter_number}: {e}", exc_info=True); return False
        try:
            with open(os.path.join(config.OUTPUT_DIR, f"chapter_{chapter_number}.txt"), 'w', encoding='utf-8') as f: f.write(final_text)
            with open(os.path.join(config.OUTPUT_DIR, f"chapter_{chapter_number}_raw_log.txt"), 'w', encoding='utf-8') as f: f.write(raw_log)
            logger.info(f"Saved chapter text & raw log for chapter {chapter_number}.")
        except IOError as e: logger.error(f"Failed to write chapter text/log files for chapter {chapter_number}: {e}", exc_info=True)
    ...
    def _update_knowledge_bases(self, chapter_number: int, final_text: str):
        # Same as your last version (error was in a sub-function)
        if not final_text: logger.warning(f"Skipping knowledge base update for ch {chapter_number}: Final text is missing."); return
    ...
    def _get_context(self, current_chapter_number: int) -> str:
        ...
        if query_embedding is None:
            ...
            if last_chap_data:
                ...
                if fallback_context: logger.info(f"Using fallback context ({len(fallback_context)} chars)."); return fallback_context[:config.MAX_CONTEXT_LENGTH]
            logger.warning("Fallback context failed."); return ""
        ...
        if (current_chapter_number - 1) > 0 and (current_chapter_number - 1) not in top_n_indices: top_n_indices.append(current_chapter_number - 1)
    ...
    def _validate_plot_arc(self, chapter_draft_text: Optional[str], chapter_number: int) -> Optional[str]:
        ...
        **CRITICAL INSTRUCTION:** Respond ONLY with `Yes` (if aligns) OR `No, because...` (if deviates, 1-2 sentence concise explanation).
        ...
        else: logger.warning(f"Plot arc validation for ch {chapter_number} ambiguous response: '{cleaned_plot_response}'. Assuming alignment."); return None
...
4. **Crucially:** Only include characters updated, newly introduced, or with a modification proposal in output JSON."""
        ...
        **Example Output (only updated chars):**\n```json\n{{ "Char1": {{ "traits": ["New"], "modification_proposal": "MODIFY status: 'Injured'", "development_in_chapter_{chapter_number}": "Fought bravely."}} }}\n```
        ...
        if not updates or not isinstance(updates, dict) or not updates: logger.info(f"LLM found no character profile JSON updates in ch {chapter_number}."); return
        ...
        for char_name, char_update in updates.items():
            ...
            if char_name not in self.character_profiles:
                new_chars_count += 1; logger.info(f"Adding new character '{char_name}' from ch {chapter_number}.")
                self.character_profiles[char_name] = {
                    ...
                    "relationships": char_update.get("relationships", {}), dev_key: char_update.get(dev_key, f"Introduced in Ch {chapter_number}."),
                ...
                updated_chars_count += 1; logger.debug(f"Updating existing character '{char_name}' from ch {chapter_number}.")
                ...
                for key, value in char_update.items():
                    ...
                    elif key == "description" and isinstance(value, str) and value.strip() and not (config.ENABLE_DYNAMIC_STATE_ADAPTATION and "modification_proposal" in char_update and "MODIFY DESCRIPTION" in char_update["modification_proposal"].upper()):
        ...
        if updated_chars_count > 0 or new_chars_count > 0: logger.info(f"Character profile JSON merge complete. Updated: {updated_chars_count}, New: {new_chars_count}.")
        ...
        try:
            ...
            if original_key.lower() == "traits":
                ...
                for match_add in re.finditer(r"ADD\s+['\"]([^'\"]+)['\"]", action_details_original_case, re.IGNORECASE):
                    trait_to_add = match_add.group(1).strip();_ = trait_to_add and current_traits_set.add(trait_to_add)
                for match_remove in re.finditer(r"REMOVE\s+['\"]([^'\"]+)['\"]", action_details_original_case, re.IGNORECASE):
                    trait_to_remove = match_remove.group(1).strip();_ = trait_to_remove and current_traits_set.discard(trait_to_remove)
                ...
                new_value = action_details_original_case.strip("'\" ")
                if new_value: profile_or_item[original_key] = new_value; logger.info(f"Applied modification to '{original_key}' for '{item_name}'. New: '{new_value[:50]}...'")
                else: logger.warning(f"Modification for '{original_key}' of '{item_name}' empty. Proposal: '{proposal}'")
    ...
    def _update_world_building(self, chapter_text: Optional[str], chapter_number: int):
        # Same as your last refactored version (with robust_merge_world_item)
        if not chapter_text: return
        text_snippet = chapter_text[:config.KNOWLEDGE_UPDATE_SNIPPET_SIZE]
        dynamic_instructions = ""
        if config.ENABLE_DYNAMIC_STATE_ADAPTATION:
...
7. **CRITICAL: Output ONLY JSON of new/updated elements.**"""
        ...
        **Example Update (only new/changed):**\n```json\n{{ "locations": {{ "Existing Loc": {{ "modification_proposal": "MODIFY atmosphere: 'Heavy.'", "elaboration_in_chapter_{chapter_number}": "Inscription found."}} }}, "history": {{ "EventX": {{ "modification_proposal": "MODIFY description: 'New detail.'" }} }} }}\n```
        ...
        if not updates or not isinstance(updates, dict) or not updates: logger.info(f"LLM found no world-building JSON updates in ch {chapter_number}."); return
        ...
        for category_key, category_updates_dict in updates.items():
            if not isinstance(category_updates_dict, dict) or not category_updates_dict: continue
            ...
            for item_name, item_update_details in category_updates_dict.items():
                if not isinstance(item_update_details, dict): logger.warning(f"Skipping invalid item_details for '{item_name}' in '{category_key}'."); continue
                ...
                if existing_item_data is None:
                    ...
                    new_item = self._robust_merge_world_item({}, item_update_details, item_log_name, chapter_number)
                    new_item[f"added_in_chapter_{chapter_number}"] = True
                    target_category_dict[item_name] = new_item
                    items_affected_count +=1
                else:
                    updated_item = self._robust_merge_world_item(existing_item_data, item_update_details, item_log_name, chapter_number)
        ...
        if items_affected_count > 0: logger.info(f"World-building JSON merge completed. {items_affected_count} items/sub-items affected.")
        else: logger.info(f"No world-building JSON items effectively updated/added in ch {chapter_number}.")

    def _robust_merge_world_item(self, target_item: Any, update_details: Dict[str, Any], item_name_for_log: str, chapter_num: int) -> Dict[str, Any]:
        # Same as your last version
        ...
        if not isinstance(target_item, dict):
            ...
            current_item_dict = {"description": str(target_item), f"updated_in_chapter_{chapter_num}": True }
        else: current_item_dict = target_item
        ...
        for key, value in update_details.items():
            ...
            if isinstance(value, dict):
                if not isinstance(target_value, dict): current_item_dict[key] = {f"added_in_chapter_{chapter_num}": True}
                current_item_dict[key] = self._robust_merge_world_item(current_item_dict[key], value, f"{item_name_for_log}.{key}", chapter_num)
                if not current_item_dict.get(f"added_in_chapter_{chapter_num}"): current_item_dict[f"updated_in_chapter_{chapter_num}"] = True
            elif isinstance(value, list):
                if not isinstance(target_value, list): current_item_dict[key] = []
                items_added_to_list = any(item_in_list not in current_item_dict[key] and not current_item_dict[key].append(item_in_list) for item_in_list in value) # .append returns None
                if items_added_to_list and not current_item_dict.get(f"added_in_chapter_{chapter_num}"): current_item_dict[f"updated_in_chapter_{chapter_num}"] = True
    ...
    def _extract_and_update_kg(self, chapter_text: Optional[str], chapter_number: int):
        # Same as your last version (uses improved parse_llm_json_response)
        if not chapter_text: logger.warning(f"Skipping KG extraction for ch {chapter_number}: Text None."); return
        logger.info(f"Extracting KG triples for chapter {chapter_number}...")
        ...
        **Example CORRECT Output (actual output is just the list):**
        ...
        [
        """
        raw_triples_json = llm_interface.call_llm(prompt, temperature=0.5, max_tokens=2000)
        ...
        for triple in parsed_triples:
            if isinstance(triple, list) and len(triple) == 3:
                ...
                if subj and pred and obj: self.db_manager.add_kg_triple(subj, pred, obj, chapter_number); added_count += 1
                else: logger.warning(f"Skipping invalid triple (empty component) in ch {chapter_number}: {triple}"); skipped_count += 1
            else: logger.warning(f"Skipping invalid triple format in KG for ch {chapter_number}: {triple}"); skipped_count += 1
        logger.info(f"Added {added_count} KG triples from ch {chapter_number}. Skipped {skipped_count}.")
    ...
    def _get_relevant_character_state_snippet(self) -> str:
        ...
        if protagonist_name and protagonist_name in sorted_char_names: sorted_char_names.insert(0, sorted_char_names.pop(sorted_char_names.index(protagonist_name)))
        for name in sorted_char_names:
            ...
            snippet_data[name] = {"desc_snippet": profile.get("description", "")[:config.PLANNING_CONTEXT_MAX_CHARS_PER_PROFILE_DESC] + "...",
    ...
    def _get_relevant_world_state_snippet(self) -> str:
        ...
        if "locations" in self.world_building and isinstance(self.world_building["locations"], dict):
             snippet_data["locations"] = list(self.world_building["locations"].keys())[:config.PLANNING_CONTEXT_MAX_LOCATIONS_IN_SNIPPET]
