...
async def save_character_profiles_to_db(profiles_data: Dict[str, Any]) -> bool:
    ...
    # Clear existing links from NovelInfo to Character before rebuilding them
    statements.append(("MATCH (ni:NovelInfo:Entity {id: $novel_id_param})-[r:HAS_CHARACTER]->(c:Character) DELETE r", {"novel_id_param": novel_id_param}))
    ...
    for record in char_results:
        ...
        if rel_results:
            for rel_rec in rel_results:
                ...
                rel_props = rel_rec.get('rel_props', {}) 
