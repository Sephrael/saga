...
async def save_character_profiles_to_db(profiles_data: Dict[str, Any]) -> bool:
    ...
    try:
        existing_char_records = await neo4j_manager.execute_read_query("MATCH (c:Character:Entity) RETURN c.name AS name")
        existing_db_char_names: Set[str] = {record['name'] for record in existing_char_records if record and record['name']}
    except Exception as e:
        logger.error(f"Failed to retrieve existing character names from DB: {e}", exc_info=True)
        return False
    ...
    for char_name, profile_dict in profiles_data.items():
        ...
        current_profile_traits: Set[str] = {str(t).strip() for t in profile_dict.get("traits", []) if isinstance(t, str) and str(t).strip()}
        ...
        statements.append((
            ...
            DELETE r
            """,
            {"char_name_val": char_name, "current_traits_list": list(current_profile_traits)}
        ))
        ...
        for key, value_str in profile_dict.items():
            if key.startswith("development_in_chapter_") and isinstance(value_str, str) and value_str.strip():
                try:
                    chap_num_int = int(key.split("_")[-1])
                    dev_event_summary = value_str.strip()
                    dev_event_id = f"dev_{char_name}_ch{chap_num_int}_{hash(dev_event_summary)}"

                    dev_event_props = {
                        "id": dev_event_id,
                        "summary": dev_event_summary,
                        KG_NODE_CHAPTER_UPDATED: chap_num_int,
                        KG_IS_PROVISIONAL: profile_dict.get(f"source_quality_chapter_{chap_num_int}") == "provisional_from_unrevised_draft",
                    }
                    ...
                    logger.warning(f"Could not parse chapter number from dev key: {key} for char {char_name}")
        ...
        if isinstance(profile_defined_rels, dict):
             target_chars_in_profile_rels = {str(k).strip() for k in profile_defined_rels.keys() if str(k).strip()}
        ...
        if isinstance(profile_defined_rels, dict):
            for target_char_name_str, rel_detail in profile_defined_rels.items():
                ...
                if isinstance(rel_detail, dict) and "chapter_added" in rel_detail:
                    try: chapter_added_val = int(rel_detail["chapter_added"])
                ...
                if isinstance(rel_detail, str) and rel_detail.strip():
                    ...
                    if rel_detail.isupper() and " " not in rel_detail: rel_type_str = rel_detail
                    ...
                    for k_rel, v_rel in rel_detail.items():
                        if isinstance(v_rel, (str, int, float, bool)) and k_rel != "type" and k_rel != "chapter_added": # chapter_added handled above
                            rel_cypher_props[k_rel] = v_rel
                
                rel_cypher_props[KG_IS_PROVISIONAL] = profile_dict.get(f"source_quality_chapter_{chapter_added_val}") == "provisional_from_unrevised_draft"
    ...
    try:
        ...
        logger.info(f"Successfully synchronized {len(all_input_char_names)} character profiles to Neo4j.")
        return True
    except Exception as e:
...
async def get_character_profiles_from_db() -> Dict[str, Any]:
    ...
    for record in char_results:
        ...
        profile["traits"] = sorted([tr['trait_name'] for tr in trait_results if tr and tr['trait_name']])
        ...
        if rel_results:
            for rel_rec in rel_results:
                ...
                rel_props_cleaned = {k:v for k,v in rel_props_full.items() 
                ...
                if 'type' in rel_props_full: rel_props_cleaned['type'] = rel_props_full['type']
                # Restore chapter_added from the rel_props_full if it was there
                if 'chapter_added' in rel_props_full: rel_props_cleaned['chapter_added'] = rel_props_full['chapter_added']
...
async def get_character_info_for_snippet_from_db(char_name: str, chapter_limit: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug(f"No detailed snippet info found for character '{char_name}' up to chapter {chapter_limit}.")
    except Exception as e:
        logger.error(f"Error fetching character info for snippet ({char_name}): {e}", exc_info=True)
    return None
