...
from .utils import world_utils # For direct access to cache utilities if needed by facade users
...
_world_persistence_service = WorldPersistenceService(world_query_service=_world_query_service)
    ...
    if full_sync:
        ...
        for cat, items_in_cat in world_items.items():
            if cat == "_overview_": # Handle overview separately if it's a WorldItem
                ...
                elif isinstance(items_in_cat, WorldItem): # If _overview_ value is directly the item
        ...
        return await _world_persistence_service.sync_full_world_state_to_db(world_data_as_dict)
    else:
...
async def fix_missing_world_element_core_fields() -> int:
    ...
    """
    return await _world_persistence_service.fix_missing_world_element_core_fields_in_db()
