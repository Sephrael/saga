...
from kg_maintainer.models import WorldItem # If WorldItem is used by utils, otherwise remove
...
def build_world_element_id(category: str, item_name: str, details: dict[str, Any]) -> str:
    """
...
def process_elaborations_for_item(
    ...
    for elab_rec in elaborations:
        ...
        if elab_rec.get("prov"): # Check for provisional flag from DB
            ...
            item_detail_dict[kg_keys.source_quality_key(chapter_val)] = ( # kg_constants
...
def initialize_item_detail_dict_from_node(
    ...
    item_detail_dict["is_provisional"] = bool(is_provisional_at_creation) # Ensure boolean
...
def populate_list_attributes_for_item(
    ...
    for attr in list_attrs:
        ...
        attr_values = [
            str(v) for v in record.get(attr, []) if v is not None
        ]
