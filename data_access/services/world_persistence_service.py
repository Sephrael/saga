...
from data_access.cypher_builders.world_cypher import (
    generate_world_element_node_cypher,  # Assuming this path
)
...
class WorldPersistenceService:
    ...
    async def sync_world_items_incremental(
        ...
        for _cat, items_in_category in world_items.items():
            ...
            for item_obj in items_in_category.values():
                if isinstance(item_obj, WorldItem) and item_obj.name and item_obj.id:
                    world_utils.update_world_name_to_id_cache(
                        item_obj.name, item_obj.id
                    )
        ...
        for category_name, items_in_category in world_items.items():
            if not isinstance(items_in_category, dict):
                logger.debug(
                    f"Skipping non-dict items for category {category_name} in incremental sync."
                )
                continue
            for item_name, item_obj in items_in_category.items():
                if not isinstance(item_obj, WorldItem):
                    logger.debug(
                        f"Skipping non-WorldItem '{item_name}' in category '{category_name}'."
                    )
                    continue
                # generate_world_element_node_cypher is expected to return a list of (query, params) tuples
                item_statements = generate_world_element_node_cypher(
                    item_obj, chapter_number
                )
        ...
        if not statements:
            logger.info(
                "No statements generated for incremental world item sync for chapter %d.",
                chapter_number,
            )
    ...
    def _collect_input_world_element_ids(
    ...
    def _prepare_world_element_props_for_sync(
        ...
        we_props["is_deleted"] = bool(
            details.get("is_deleted", False)
        )  # Ensure it's set
                ...
                """,
                    {
                        ...
                        if current_values
                            ...
                            "__dummy_value_to_prevent_deleting_all__"
                        ],  # Handle empty list
                    },
                )
            )
    ...
    async def _generate_orphan_deletion_statements_for_sync(
        ...
        try:
            ...
            if self._world_query_service:
                ...
                logger.warning(
                    "WorldQueryService not available for orphan check, fetching IDs directly."
                )
                existing_ids = await self._fetch_existing_world_element_ids_from_db()

        except Exception as exc:
            logger.error(
                f"Failed to retrieve existing WorldElement IDs for orphan check: {exc}",
                exc_info=True,
            )
            raise
        ...
        for category, items in world_data_dict.items():
            ...
            for name, details in items.items():
                if not isinstance(details, dict) or name.startswith(
                    ("_", kg_keys.SOURCE_QUALITY_PREFIX)
                ):
                    continue

                we_id = world_utils.build_world_element_id(category, name, details)
                props = self._prepare_world_element_props_for_sync(
                ...
                statements.extend(
                    self._generate_list_value_statements_for_sync(we_id, details)
                )
                statements.extend(
                    self._generate_elaboration_statements_for_sync(we_id, details, name)
                )
        return statements
    ...
    async def _build_all_sync_statements(
        ...
        try:
            orphan_statements = (
                await self._generate_orphan_deletion_statements_for_sync(
                    world_data_dict
                )
            )
            ...
            logger.error(
                f"Failed to generate orphan deletion statements during full sync build: {exc}",
                exc_info=True,
            )
            # Decide on error handling: continue without orphan deletion or raise
            pass
        statements.extend(
            self._generate_world_element_statements_for_sync(world_data_dict, wc_id)
        )
        statements.append(self._cleanup_orphan_value_nodes_statement())
        return statements

    async def sync_full_world_state_to_db(
        self, world_data_as_dict: dict[str, Any]
    ) -> bool:
        """
        ...
        """
        logger.info(
            "WorldPersistenceService: Synchronizing full world building data to Neo4j..."
        )
        ...
        for category, items in world_data_as_dict.items():
            ...
            for name, details in items.items():
                if isinstance(details, dict):
                    item_id = world_utils.build_world_element_id(
                        category, name, details
                    )
        ...
        if isinstance(overview_details, dict) and overview_details.get("id"):
            world_utils.update_world_name_to_id_cache(
                "_overview_", overview_details["id"]
            )
        ...
        all_statements = await self._build_all_sync_statements(
            world_data_as_dict, wc_id, novel_id
        )
        ...
        try:
            ...
            logger.info(
                "WorldPersistenceService: Successfully synchronized full world building data."
            )
            return True
        except Exception as e:
            logger.error(
                f"WorldPersistenceService: Error synchronizing full world data: {e}",
                exc_info=True,
            )
            return False
    ...
    async def fix_missing_world_element_core_fields_in_db(self) -> int:
        ...
        for rec in results:
            ...
            if not current_name:
                current_name = (
                    w_id.split("_", 1)[-1].replace("_", " ").title()
                    if w_id
            ...
            if (
                not current_id and current_name and current_category
            ):  # Must have name/cat to build ID
                props_to_set["id"] = world_utils.build_world_element_id(
        ...
        try:
            await neo4j_manager.execute_cypher_batch(statements)
            logger.info(
                "WorldPersistenceService: Fixed missing core fields for %d WorldElements.",
                len(statements),
            )
        except Exception as exc:
            logger.error(
    ...
    async def remove_world_element_trait_aspect_from_db(
        self, element_id: str, trait_value: str
    ) -> bool:
        """
        ...
        try:
            ...
            await neo4j_manager.execute_write_query(
                cleanup_query, {"trait": trait_value}
            )
            logger.info(
                "Removed trait '%s' from world element %s.", trait_value, element_id
            )
            return True
        except Exception as exc:
            logger.error(
