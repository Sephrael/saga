...
async def comprehensive_chapter_evaluation(
    ...
    if plot_point_focus is None: logger.warning(f"Plot point focus not available for Ch {chapter_number} during comprehensive evaluation.")
...
async def evaluate_chapter_draft_logic(agent, draft_text: str, chapter_number: int, previous_chapters_context: str) -> EvaluationResult:
    ...
    if chapter_number > 1:
        ...
        else: logger.warning(f"Could not perform coherence check for ch {chapter_number} (missing current or previous embedding).")
    else: 
    ...
    # MODIFIED: Call new parsing utility
    parsed_problems_from_llm = _parse_llm_evaluation_output(llm_eval_text_output, chapter_number)
