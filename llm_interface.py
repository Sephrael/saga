...
def truncate_text_by_tokens(text: str, model_name: str, max_tokens: int, truncation_marker: str = "\n... (truncated)") -> str:
    ...
    if not encoder:
        ...
        if len(text) > max_chars:
            ...
            if effective_max_chars < 0: effective_max_chars = 0
...
class LLMService:
        ...
        for attempt in range(config.LLM_RETRY_ATTEMPTS):
            ...
            try:
                ...
                if primary_key in data and isinstance(data[primary_key], list):
                    embedding = self._validate_embedding(data[primary_key], config.EXPECTED_EMBEDDING_DIM, config.EMBEDDING_DTYPE)
                    ...
                    for key, value in data.items(): 
                        if isinstance(value, list) and all(isinstance(item, (float, int)) for item in value):
                            embedding = self._validate_embedding(value, config.EXPECTED_EMBEDDING_DIM, config.EMBEDDING_DTYPE)
                            if embedding is not None:
    ...
    def _log_llm_usage(self, model_name: str, usage_data: Optional[Dict[str, int]], async_mode: bool = False, streamed: bool = False):
                ...
                try:
                    if stream_to_disk:
                        ...
                        try:
                            ...
                            self._log_llm_usage(current_model_to_try, current_usage_data, async_mode=True, streamed=True)
                            if auto_clean_response:
                                final_text_response = self.clean_model_response(final_text_response)
                            return final_text_response, current_usage_data
