...
class LLMService:
        ...
        for retry_attempt in range(settings.LLM_RETRY_ATTEMPTS):
            try:
                ...
                if (
                    ...
                    logger.error(
                        f"Async LLM ('{model_name}'): Client-side error {exc.response.status_code}. Aborting retries for this model call."
                    )
                ...
                if (
                    ...
                    and last_exc
                    is not None  # Only cleanup if there was an error in this attempt
                ):
                    try:
                        logger.info(
    ...
    async def _async_call_llm(
        ...
        async with self._semaphore:
            ...
            (primary_payload, primary_prompt_tokens, primary_temp, primary_max_out) = (
            ...
            if (
                ...
                and allow_fallback
                and settings.FALLBACK_GENERATION_MODEL
            ):
                ...
                if (
                    ...
                    and last_exception.response
                    and 400 <= last_exception.response.status_code < 500
            ...
            elif last_exception is not None and (
                not allow_fallback or not settings.FALLBACK_GENERATION_MODEL
            ):
                logger.warning(
