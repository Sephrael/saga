...
class DatabaseManager:
    ...
    def _deserialize_embedding(self, blob: bytes, dtype_str: str, shape_str: str) -> Optional[np.ndarray]:
        ...
        try:
            ...
            dtype = np.dtype(dtype_str) # Use config.EMBEDDING_DTYPE if dtype_str is problematic
            embedding = np.frombuffer(blob, dtype=dtype).reshape(shape)
            return embedding
        except (json.JSONDecodeError, TypeError, ValueError, AttributeError) as e: # Added AttributeError for np.dtype
    ...
    def save_chapter_data(self, chapter_number: int, text: str, raw_text: str, summary: Optional[str], embedding: Optional[np.ndarray]):
        ...
        if embedding is not None and isinstance(embedding, np.ndarray) and embedding.size > 0:
            try:
                ...
                if embedding_to_save.ndim == 0: embedding_to_save = embedding_to_save.reshape(1) # Handle 0-dim arrays
            ...
            logger.warning(f"Chapter {chapter_number} has no valid embedding provided or embedding is empty. Embedding will not be saved.")
        ...
        try:
            with self._get_connection() as conn:
                ...
                if embedding_blob and dtype_str and shape_str: # Ensure all parts are valid
                    cursor.execute(
                    ...
                    logger.debug(f"Removed any existing embedding entry for chapter {chapter_number} due to invalid new embedding.")
                conn.commit()
        ...
        try:
            ...
            for row in results:
                ...
                if embedding is not None:
                    ...
                    logger.warning(f"Failed to load/deserialize embedding for past chapter {row['chapter_number']}. Skipping.")
        except sqlite3.Error as e:
    ...
    def add_kg_triple(self, subject: str, predicate: str, obj: str, chapter_added: int, confidence: float = 1.0):
        ...
        if not all([subj_clean, pred_clean, obj_clean]) or chapter_added <= 0:
            logger.warning(f"Attempted to add invalid or empty triple: S='{subj_clean}', P='{pred_clean}', O='{obj_clean}', Chap={chapter_added}")
            return
    ...
    def query_kg(self, subject: Optional[str] = None, predicate: Optional[str] = None, obj: Optional[str] = None, chapter_limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        """
        query_parts = ["SELECT id, subject, predicate, object, chapter_added, confidence, timestamp FROM knowledge_graph WHERE 1=1"]
    ...
    def get_most_recent_value(self, subject: str, predicate: str, chapter_limit: Optional[int] = None) -> Optional[str]:
        ...
        if not subject.strip() or not predicate.strip():
            logger.warning(f"Attempted get_most_recent_value with empty subject or predicate: S='{subject}', P='{predicate}'")
            return None
