    ...
    async def get_context(
        ...
        if prev_chapter <= 0:
            ...
            if request.agent_hints and request.agent_hints.get(
                ...
                hint = request.agent_hints["previous_chapter_end_state"]
                if isinstance(hint, ChapterEndState):
                    state = hint
                else:
                    try:
                        ...
                        logger.error(
                            "Failed to parse previous end state from hints",
                            exc_info=True,
                        )
            if state is None:
                try:
                    data = await self.chapter_queries.get_chapter_data_from_db(
                        prev_chapter
                    )
                except Exception as exc:  # pragma: no cover - log and return empty
                    logger.error(
