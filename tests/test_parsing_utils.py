...
class TestRdfTripleParsing(unittest.TestCase):
    ...
    def test_parse_simple_turtle(self):
        ...
        parsed_triples = parse_rdf_triples_with_rdflib(turtle_input, rdf_format="turtle")

        self.assertEqual(len(parsed_triples), expected_triples_count)

        jax_alias_triple = next((t for t in parsed_triples if t["subject"]["name"] == "Jax" and t["predicate"] == "hasAlias"), None)
        ...
        if jax_alias_triple:
            ...
            self.assertEqual(jax_alias_triple["subject"]["type"], "Character", "Jax type should be Character from rdf:type")

        jax_livesin_triple = next((t for t in parsed_triples if t["subject"]["name"] == "Jax" and t["predicate"] == "livesIn"), None)
        ...
        if jax_livesin_triple:
            ...
            if jax_livesin_triple["object_entity"]:
                self.assertEqual(jax_livesin_triple["object_entity"]["name"], "Hourglass Curios")
                self.assertEqual(jax_livesin_triple["object_entity"]["type"], "Location")

        jax_type_triple = next((t for t in parsed_triples if t["subject"]["name"] == "Jax" and t["predicate"] == "type" and t["object_entity"]["name"] == "Character"), None)
        self.assertIsNotNone(jax_type_triple, "Jax rdf:type Character triple not found")

        lila_type_triple = next((t for t in parsed_triples if t["subject"]["name"] == "Lila" and t["predicate"] == "type" and t["object_entity"]["name"] == "Character"), None)
    ...
    def test_invalid_turtle(self):
        ...
        parsed_triples = parse_rdf_triples_with_rdflib(invalid_turtle, rdf_format="turtle")
        self.assertEqual(len(parsed_triples), 0)
