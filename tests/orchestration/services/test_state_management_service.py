...
from chapter_generation import ContextProfileName # For context building call
...
class TestStateManagementService(unittest.IsolatedAsyncioTestCase): # Use IsolatedAsyncioTestCase for async tests
    ...
    @patch('orchestration.services.state_management_service.chapter_repository', new_callable=MagicMock)
    @patch('orchestration.services.state_management_service.plot_queries', new_callable=MagicMock)
    async def test_async_init_state_success(self, mock_plot_queries, mock_chapter_repo):
        ...
        mock_plot_queries.get_plot_outline_from_db = AsyncMock(return_value={"title": "Test Novel", "plot_points": [{"summary": "PP1"}]})
        mock_plot_queries.get_completed_plot_points = AsyncMock(return_value={"PP1_focus"})
    ...
    @patch('orchestration.services.state_management_service.chapter_repository', new_callable=MagicMock)
    @patch('orchestration.services.state_management_service.plot_queries', new_callable=MagicMock)
    async def test_async_init_state_plot_outline_exception(self, mock_plot_queries, mock_chapter_repo):
        mock_chapter_repo.load_chapter_count = AsyncMock(return_value=2)
        mock_plot_queries.ensure_novel_info = AsyncMock()
        mock_plot_queries.get_plot_outline_from_db = AsyncMock(side_effect=Exception("DB error"))
        ...
        self.assertIsNotNone(self.service.plot_outline) # Should be an empty PlotOutline
    ...
    async def test_refresh_plot_outline(self):
        ...
        with patch('orchestration.services.state_management_service.plot_queries.get_plot_outline_from_db', AsyncMock(return_value={"title": "Refreshed"})) as mock_get_db:
    ...
    async def test_refresh_knowledge_cache(self):
        ...
        self.mock_orchestrator.knowledge_cache = KnowledgeCache(characters={"char1": {"name": "Char1"}}, world={"loc1": {"name": "Loc1"}})

        await self.service.refresh_knowledge_cache()

        self.mock_orchestrator.knowledge_service.refresh_knowledge_cache.assert_called_once()
        self.assertEqual(self.service.knowledge_cache.characters, {"char1": {"name": "Char1"}})
        self.assertEqual(self.service.knowledge_cache.world, {"loc1": {"name": "Loc1"}})


    @patch('orchestration.services.state_management_service.chapter_repository.get_chapter_data', new_callable=AsyncMock)
    async def test_load_previous_end_state(self, mock_get_chapter_data):
        # Test chapter > 0
        mock_end_state_json = ChapterEndState(summary="End state sum", final_prompt="Final prompt", characters_present=["c1"]).model_dump_json()
        ...
        self.service.chapter_zero_end_state = ChapterEndState(summary="Ch0 Sum", final_prompt="FP0", characters_present=[])
        end_state_0_cached = await self.service.load_previous_end_state(0)
        self.assertEqual(end_state_0_cached.summary, "Ch0 Sum")
        mock_get_chapter_data.assert_called_with(3) # Should not be called again for 0 if cached

        # Test chapter 0 (not cached, needs loading)
        self.service.chapter_zero_end_state = None
        mock_get_chapter_data.reset_mock()
        mock_get_chapter_data.return_value = {"end_state_json": ChapterEndState(summary="Loaded Ch0", final_prompt="LFP0", characters_present=[]).model_dump_json()}
    ...
    @patch('orchestration.services.state_management_service.plot_queries', new_callable=MagicMock)
    @patch('orchestration.services.state_management_service.neo4j_manager.driver', new_callable=MagicMock)
    async def test_update_state_after_chapter_finalization(self, mock_driver, mock_plot_queries):
        mock_driver.is_connected = True # Simulate connected driver
        self.service.plot_outline = PlotOutline(title="Test", plot_points=[{"summary":"pp1"}, {"summary":"pp2"}])
        ...
        end_state_mock = ChapterEndState(summary="Chapter Done", final_prompt="FP", characters_present=[])
        ...
        with patch('orchestration.services.state_management_service.get_plot_point_info', return_value=("pp1_focus", 0)):
             with patch('orchestration.services.state_management_service.settings.PLOT_POINT_CHAPTER_SPAN', 1):
                await self.service.update_state_after_chapter_finalization(1, end_state_mock)
    ...
    def test_store_pending_fill_ins(self):
        ...
        self.mock_orchestrator.context_service.llm_fill_chunks = [mock_chunk1, mock_chunk2, mock_chunk3]
    ...
    def test_getters_setters(self):
        ...
        self.service.knowledge_cache = kc # Direct set for simplicity if no setter logic
