...
class MockOrchestrator:
    def __init__(self):
        ...
        self.state_manager.get_plot_outline = MagicMock(
            return_value={"title": "Test Novel"}
        )
...
class TestTokenManagementService(unittest.TestCase):
    def setUp(self):
        ...
        self.service = TokenManagementService(
            self.mock_display_manager, self.mock_orchestrator_ref
        )
    ...
    def test_accumulate_tokens_records_and_updates_display(
        self, mock_update_display, mock_record_usage
    ):
        ...
        self.service.token_accountant.record_usage.assert_called_once_with(
            stage.value, usage_data
        )
        self.assertEqual(self.service.total_tokens_generated_this_run, 100)
        mock_update_display.assert_called_once_with(
            chapter_num=chapter_num, step=step_display
        )
    ...
    def test_update_rich_display_calls_display_manager(self, mock_display_update):
        ...
        self.mock_orchestrator_ref.state_manager.get_plot_outline.return_value = (
    ...
    def test_reset_tokens_for_new_run(self, mock_update_display):
        ...
        self.service.token_accountant.record_usage(
            "some_stage", {"total_tokens": 1000}
        )  # Simulate usage
        ...
        mock_update_display.assert_called_once_with(
            step="New Run Initialized - Tokens Reset"
        )
