...
class WorldItem:
    ...
    def from_dict(cls, category: str, name: str, data: Dict[str, Any]) -> "WorldItem":
        ...
        if not name or not isinstance(name, str) or not name.strip():
            raise ValueError(f"WorldItem name must be a non-empty string (for category '{category}').")
        ...
        if not normalized_id_category: normalized_id_category = "unknown_category"
        if not normalized_id_name: normalized_id_name = "unknown_name"
