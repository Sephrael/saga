...
def initialize_new_character_profile(char_name: str, char_update: CharacterProfile, chapter_number: int) -> CharacterProfile:
    ...
    new_profile = CharacterProfile(
        name=char_name,
        description=data.get("description", f"A character newly introduced in Chapter {chapter_number}."),
        traits=sorted({t for t in data.get("traits", []) if isinstance(t, str) and t.strip()}),
        relationships=data.get("relationships", {}),
        status=data.get("status", "Newly introduced"),
        updates={dev_key: data.get(dev_key, f"Character '{char_name}' introduced in Chapter {chapter_number}.")},
    )
...
def merge_character_profile_updates(profiles: Dict[str, CharacterProfile], updates: Dict[str, CharacterProfile], chapter_number: int, from_flawed_draft: bool) -> None:
    ...
    for name, update in updates.items():
        ...
        if name not in profiles:
            profiles[name] = initialize_new_character_profile(name, update, chapter_number)
            continue
        profile = profiles[name]
        prof_dict = profile.to_dict()
        modified = False
        for key, val in data.items():
            if key in {"modification_proposal", provisional_key} or key.startswith("development_in_chapter_"):
                continue
            if key == "traits" and isinstance(val, list):
                new_traits = sorted(set(profile.traits).union({t for t in val if isinstance(t, str) and t.strip()}))
...
def merge_world_item_updates(world: Dict[str, Dict[str, WorldItem]], updates: Dict[str, Dict[str, WorldItem]], chapter_number: int, from_flawed_draft: bool) -> None:
    ...
    for category, cat_updates in updates.items():
        ...
        for name, update in cat_updates.items():
            ...
            if name not in world[category]:
                ...
                world[category][name].properties.setdefault(f"added_in_chapter_{chapter_number}", True)
                continue
            item = world[category][name]
            item_props = item.to_dict()
            for key, val in data.items():
                if key in {provisional_key, "modification_proposal"} or key.startswith(("updated_in_chapter_", "added_in_chapter_", "source_quality_chapter_")):
                    if key.startswith("elaboration_in_chapter_") and isinstance(val, str) and val.strip():
