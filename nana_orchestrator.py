...
class NANA_Orchestrator:
    def __init__(self):
        ...
        if RICH_AVAILABLE and config.ENABLE_RICH_PROGRESS:
            ...
            self.rich_live = Live(
                ...
                redirect_stdout=False, # Allow regular prints to go to console too if needed, or set to True
                redirect_stderr=False
            )
        else:
            logger.info("Rich library not available or ENABLE_RICH_PROGRESS is False. Progress will be shown via standard logs.")
        utils.load_spacy_model_if_needed() # Ensure spaCy is loaded early
    ...
    def _update_rich_display(self, chapter_num: Optional[int] = None, step: Optional[str] = None):
        ...
        # self.rich_live.update(Panel(self.rich_status_group, title="SAGA NANA Progress", border_style="blue", expand=True)) # Not needed if modifying Text objects in place
    ...
    def _accumulate_tokens(self, operation_name: str, usage_data: Optional[Dict[str, int]]):
        if usage_data and isinstance(usage_data.get("completion_tokens"), int):
            ...
            # Log outside Rich update for persistent record
            logger.info(
    ...
    async def _prepopulate_kg_if_needed(self):
        ...
        pp_result = await neo4j_manager.execute_read_query(pp_check_query)
    ...
    async def perform_deduplication(self, text_to_dedup: str, chapter_number: int) -> Tuple[str, int]:
        ...
        Returns the de-duplicated text and the number of characters removed.
        """
        logger.info(f"NANA: Performing de-duplication for Chapter {chapter_number}...")
        
        segment_level_dedup = "paragraph" 
        use_semantic_dedup = False # Using normalized string comparison as per recommendation
    ...
    async def run_chapter_generation_process(self, chapter_number: int) -> Optional[str]:
        ...
        if config.ENABLE_AGENTIC_PLANNING and chapter_plan is None:
            logger.warning(f"NANA: Ch {chapter_number}: Planning Agent failed or plan invalid. Proceeding with plot point focus only for drafting.")
        ...
        if final_embedding is None:
            logger.error(f"NANA CRITICAL: Failed to generate embedding for final text of Chapter {chapter_number}. Text saved to file system only.")
        ...
        self.world_building = self.novel_props_cache['world_building']   # Sync back
        self._update_novel_props_cache()
    ...
    async def run_novel_generation_loop(self):
        ...
        try:
            ...
            if self.rich_live and self.rich_live.is_started: # type: ignore
                await asyncio.sleep(0.1) # Ensure final update renders
                self.rich_live.stop()
...
def setup_logging_nana():
    ...
    # Add RichHandler if available and enabled, otherwise add standard StreamHandler
    if RICH_AVAILABLE and config.ENABLE_RICH_PROGRESS:
        # Configure RichHandler not to interfere with the Live display's console capture
        rich_handler = RichHandler(
            ...
            # The console bit above is an attempt to reuse the Live console if RichHandler is added after Live.
            # Simpler is to ensure Live captures std logging.
        )
        # If Live is redirecting stdout/stderr, RichHandler output will also be captured by Live.
        # If not, RichHandler will print to the original console.
    ...
    try:
        ...
        if neo4j_manager.driver is not None:
            ...
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # If loop is running, can't run another asyncio.run, schedule as task or handle differently
                    # For simplicity here, assume it's okay or will be handled by orchestrator.close()
                    asyncio.ensure_future(_close_driver_main()) # Fire and forget if loop is running
                else:
