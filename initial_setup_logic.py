...
async def generate_plot_outline_logic(
    ...
    **kwargs,
    ...
    if user_supplied_data is not None:
        ...
        plot_outline, character_profiles, _ = _populate_state_from_user_data(
    ...
    if needs_llm_for_core_plot:
        ...
        if plot_outline:
            for display_key_normalized, internal_key in PLOT_OUTLINE_KEY_MAP.items():
                ...
                if user_val is not None and not utils._is_fill_in(user_val):
                    context_from_user_input += f"  - {display_key_title_case}: {user_val}\n"
        ...
        in_pure_llm_scratch_mode = not user_supplied_data and not any(plot_outline.values())
        ...
        if unhinged_mode and in_pure_llm_scratch_mode:
            ...
            base_elements_for_outline["genre"] = plot_outline.get("genre") if not utils._is_fill_in(plot_outline.get("genre")) else config.CONFIGURED_GENRE
            base_elements_for_outline["theme"] = plot_outline.get("theme") if not utils._is_fill_in(plot_outline.get("theme")) else config.CONFIGURED_THEME
            base_elements_for_outline["setting_description"] = plot_outline.get("setting_description") if not utils._is_fill_in(plot_outline.get("setting_description")) else config.CONFIGURED_SETTING_DESCRIPTION
            base_elements_for_outline["protagonist_name"] = plot_outline.get("protagonist_name") if not utils._is_fill_in(plot_outline.get("protagonist_name")) else default_protagonist_name
        ...
        try:
            ...
            if isinstance(parsed_llm_response, dict):
                ...
                raise json.JSONDecodeError("LLM output was not a JSON object.", cleaned_outline_text, 0)
        except json.JSONDecodeError as e:
            logger.error(f"Failed to decode or validate LLM JSON for plot outline: {e}. Falling back to default.")
            plot_outline = _create_default_plot(default_protagonist_name, base_elements_for_outline, unhinged_mode and in_pure_llm_scratch_mode)
    
    final_protagonist_name = plot_outline.get("protagonist_name", default_protagonist_name)
    if final_protagonist_name not in character_profiles:
        character_profiles[final_protagonist_name] = CharacterProfile(name=final_protagonist_name)
    ...
    prot_profile_ref.updates["character_arc_summary"] = plot_outline.get("character_arc", "")
    ...
    return plot_outline, character_profiles, accumulated_usage_data if llm_was_called else None
...
async def generate_world_building_logic(
    ...
    if not world_building:
        ...
        for category_internal, items_or_details in world_building.items():
            ...
            if isinstance(items_or_details, dict):
                if any(utils._is_fill_in(v) for v in items_or_details.values() if not isinstance(v, (dict, list))):
                    needs_llm_for_world = True
                    break
                for item_detail in items_or_details.values():
                    if isinstance(item_detail, dict) and any(utils._is_fill_in(v) for v in item_detail.values()):
                        needs_llm_for_world = True
                        break
    ...
    if usage_data:
        accumulated_usage_data.update({k: accumulated_usage_data.get(k, 0) + v for k, v in usage_data.items()})
    ...
    try:
        ...
        if isinstance(parsed_llm_json_response, dict):
            ...
            for cat_key, cat_content in parsed_llm_json_response.items():
                internal_cat_name = WORLD_CATEGORY_MAP_NORMALIZED_TO_INTERNAL.get(cat_key)
                if internal_cat_name:
                    if internal_cat_name == "_overview_":
                         world_building[internal_cat_name] = {"_overview_": WorldItem.from_dict(internal_cat_name, "_overview_", cat_content)}
                    else:
                        world_building[internal_cat_name] = {
                            item_name: WorldItem.from_dict(internal_cat_name, item_name, item_details)
                            for item_name, item_details in cat_content.items()
                        }
            logger.info("Successfully processed LLM-generated JSON world-building into state.")
    except json.JSONDecodeError as e:
        logger.error(f"Failed to decode world-building JSON: {e}. Using existing/default data.")
