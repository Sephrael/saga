...
async def generate_chapter_draft_logic(
    ...
    if not plot_point_focus:
        ...
        logger.warning(
            f"Plot point focus was None for Ch {chapter_number} draft generation. Using generic fallback."
        )
    ...
    if config.ENABLE_AGENTIC_PLANNING:
        if chapter_plan and isinstance(chapter_plan, list):
            ...
            logger.info(
                f"Using detailed scene plan (plain text) for Ch {chapter_number} draft generation."
            )
        else:
            plan_section_for_prompt_parts.append(
                f"**Chapter Plan Note:** No detailed scene plan available. Rely on the Overall Plot Point Focus.\n**Overall Plot Point Focus for THIS Chapter:** {plot_point_focus}\n"
            )
    else:
        plan_section_for_prompt_parts.append(
            f"**Chapter Plan Note:** Detailed agentic planning is disabled. Rely on the Overall Plot Point Focus.\n**Overall Plot Point Focus for THIS Chapter:** {plot_point_focus}\n"
        )
    ...
    prompt_lines.extend(
        [
            ...
            else "No specific world building notes provided for this chapter's context.",
            "```",
            ...
            else "No specific character profiles provided for this chapter's context.",
            "```",
            "**Hybrid Context (Semantic Context for Flow & KG Facts for Canon):**",
            "--- BEGIN HYBRID CONTEXT ---",
            hybrid_context
            if hybrid_context.strip()
            else "No previous context (e.g., this is Chapter 1 or context retrieval failed).",
            "--- END HYBRID CONTEXT ---",
            "",
    ...
    logger.info(
        f"Calling LLM ({config.DRAFTING_MODEL}) for Ch {chapter_number} draft. Target minimum length: {config.MIN_ACCEPTABLE_DRAFT_LENGTH} chars."
    )
    ...
    if not raw_llm_text_for_log:
        logger.error(
            f"LLM returned no content for Ch {chapter_number} draft (primary and potential fallback failed)."
        )
        return None, None
    ...
    if not cleaned_text or len(cleaned_text) < 50:
        logger.error(
            f"Ch {chapter_number} draft has virtually no content after cleaning ({len(cleaned_text or '')} chars). Raw LLM output snippet: '{raw_llm_text_for_log[:200]}...'"
        )
    ...
    logger.info(
        f"Generated initial draft for ch {chapter_number} (Length: {len(cleaned_text)} chars)."
    )
