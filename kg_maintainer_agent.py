...
CHAR_UPDATE_RELATIONSHIP_HANDLING = {} # Can be configured if relationships are comma-separated
...
def _merge_world_item_updates_into_state_internal(
    ...
    for category_key, category_updates_dict in updates_from_llm.items():
        if not isinstance(category_updates_dict, dict) or not category_updates_dict:
            ...
            continue
        if category_key not in world_building_dict_to_update: world_building_dict_to_update[category_key] = {}
...
async def _prepopulate_kg_from_dicts_internal(
    ...
    for char_name, profile in character_profiles.items():
        ...
        # Ensure 'name' is part of the properties for the SET c += $props if it's not used in MERGE's pattern directly for other properties
        char_props_for_set = char_props.copy()
        char_props_for_set['name'] = char_name # Ensure name is set if not already there
        ...
        if isinstance(profile.get("relationships"), dict):
            for target_name, rel_detail in profile["relationships"].items():
                ...
                cypher_statements.append((
                    "MATCH (c1:Character:Entity {name: $char_name1}) MERGE (c2:Character:Entity {name: $char_name2}) ON CREATE SET c2.description = 'Auto-created via relationship from ' + $char_name1, c2.name = $char_name2 MERGE (c1)-[r:DYNAMIC_REL {type:$rel_type_val}]->(c2) SET r += $rel_props_val",
                ...
                if isinstance(details.get(list_prop_name), list):
                    for val_item in details[list_prop_name]:
                        if isinstance(val_item, str):
                            ...
                            cypher_statements.append((f"MATCH (we:WorldElement {{id: $we_id}}) MERGE (v:ValueNode {{value: $val_item_value, type: $value_node_type}}) MERGE (we)-[:{rel_name}]->(v)", {"we_id": we_id, "val_item_value": val_item, "value_node_type": list_prop_name}))
...
class KGMaintainerAgent:
        ...
        for i, start_match in enumerate(character_block_starts):
            ...
            if individual_char_block_text:
                parsed_char_data = parse_key_value_block(
                    individual_char_block_text, CHAR_UPDATE_KEY_MAP, CHAR_UPDATE_LIST_INTERNAL_KEYS,
                ...
                if "relationships" in parsed_char_data and isinstance(parsed_char_data["relationships"], list):
                    ...
                    for rel_str in parsed_char_data["relationships"]:
                        ...
                        elif isinstance(rel_str, str) and rel_str.strip():
                ...
                elif specific_dev_key_from_llm and parsed_char_data.get(specific_dev_key_from_llm):
                    if specific_dev_key_from_llm != dev_key:
    ...
    def _parse_unified_world_updates(self, text_block: str, chapter_number: int) -> Dict[str, Any]:
        parsed_data = parse_hierarchical_structured_text(
            text_block, WORLD_UPDATE_CATEGORY_PATTERN, WORLD_UPDATE_ITEM_PATTERN,
            WORLD_UPDATE_DETAIL_KEY_MAP, WORLD_UPDATE_DETAIL_LIST_INTERNAL_KEYS, "_overview_"
        )
...
"""
        ...
        for i in range(1, len(sections), 2):
            ...
            if "character" in header_name_raw: parsed_sections["character_updates"] = content_text
            elif "world" in header_name_raw: parsed_sections["world_updates"] = content_text
            elif "kg triples" in header_name_raw: parsed_sections["kg_triples"] = content_text
        ...
        final_extraction = {"character_updates": char_updates_dict, "world_updates": world_updates_dict, "knowledge_triples": kg_triples_list}
        logger.info(f"Unified knowledge extraction for Ch {chapter_number} complete. Chars: {len(final_extraction['character_updates'])}, World cats: {len(final_extraction['world_updates'])}, KG Triples: {len(final_extraction['knowledge_triples'])}.")
        if not char_updates_dict and not world_updates_dict and not kg_triples_list and cleaned_extraction_text.strip():
            logger.warning(f"Unified extraction for Ch {chapter_number} empty despite text. Raw: '{cleaned_extraction_text[:500]}...'")
    ...
    async def extract_and_merge_knowledge(
        ...
        if not final_chapter_text:
            logger.warning(f"Skipping knowledge updates for ch {chapter_number}: Final text empty.")
            return
        logger.info(f"KGMaintainerAgent updating knowledge for ch {chapter_number} (Flawed draft: {is_from_flawed_draft})...")
        extraction_results = await self._perform_unified_knowledge_extraction(
            novel_props_mutable, final_chapter_text, chapter_number
        )
        ...
        else: logger.warning(f"No valid character updates from extraction for ch {chapter_number}.")
        ...
        else: logger.warning(f"No valid world-building updates from extraction for ch {chapter_number}.")
            ...
            for triple_any in kg_triples_list:
                if isinstance(triple_any, list) and len(triple_any) == 3:
                    ...
                    if subj and pred and obj_val:
                        ...
                        added_count += 1
                    else: skipped_count += 1; logger.warning(f"Skipping invalid KG triple (empty component): {triple_any}")
                else: skipped_count += 1; logger.warning(f"Skipping invalid KG triple format: {triple_any}")
            if kg_add_tasks: await asyncio.gather(*kg_add_tasks)
            logger.info(f"KG update for ch {chapter_number}: Added {added_count} triples, skipped {skipped_count}.")
        else: logger.info(f"No KG triples from extraction for ch {chapter_number}.")
        logger.info(f"Unified knowledge processing complete for ch {chapter_number}.")
    ...
    async def heal_canon(self, identified_contradictions: List[Dict]) -> bool:
        ...
        for contradiction in identified_contradictions:
            logger.info(f"Attempting to address contradiction: {contradiction}")
        return False